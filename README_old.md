...
Le programme génère automatiquement **20 images** en faisant tourner la lumière autour de la scène :

```bash
python3 raytracing_helped.py
```
...
```bash
convert -delay 10 -loop 0 frame_*.ppm animation.gif
```

Options :
- `-delay 10` : vitesse de l’animation
- `-loop 0` : boucle infinie

---

### (Optionnel) Réduction de taille

```bash
convert frame_*.ppm -resize 300x300 -colors 128 -delay 10 -loop 0 animation.gif
```
...
Ou directement dans un navigateur web.
