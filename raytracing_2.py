...
def vector_add(a, b):
    """Add two vectors as tuples"""
    return tuple(x + y for x, y in zip(a, b))

def vector_subtract(a, b):
    """Subtract two vectors as tuples"""
    return tuple(x - y for x, y in zip(a, b))

def vector_multiply_scalar(v, s):
    """Multiply vector by scalar (tuple version)"""
    return tuple(x * s for x in v)

def vector_dot(a, b):
    """Dot product of two vectors as tuples"""
    return sum(x * y for x, y in zip(a, b))

def vector_length(v):
    """Length of a vector as tuple"""
    return math.sqrt(vector_dot(v, v))

def vector_normalize(v):
    """Normalize a vector as tuple"""
    l = vector_length(v)
    if l == 0:
        return v
...
def main():
    ...
    if args.animate:
        ...
        try:
            # check=True lèvera une exception si la commande échoue
            ...
            if not args.keep_ppm:
                ...
                # Sur Windows, ce serait: subprocess.run("del frame_*.ppm", shell=True)
